...
export const getFormattedDecimalGenome = (genome: Genome): DecimalGenome => {
  return genome.map((gene) => {
    const { firstCharacter, restOfString } = splitFirstCharacter(gene.weight)
    ...
    const decimalGene: Gene = {
      ...
      weight: firstCharacter === '0' ? (-weight).toString() : weight
    }

    return {
      sourceType: Number(decimalGene.sourceType),
      sourceId: Number(decimalGene.sourceType) === SOURCE_TYPE_INPUT_INTERNAL_NEURON
        ? Number(decimalGene.sourceId) % config.innerNeurons
        : Number(decimalGene.sourceId) % NUM_SENSORY_NEURONS,
      sinkType: Number(decimalGene.sinkType),
      sinkId: Number(decimalGene.sinkType) === SINK_TYPE_INTERNAL_NEURON
